...
public class MySQLTableCreation {
	public static void main(String[] args) {
		try {
			...
			Class.forName("com.mysql.jdbc.Driver").newInstance(); // reflection 注册driver
			conn = DriverManager.getConnection(MySQLDBUtil.URL); // 建立和指定数据库的连接
...
//			String sql = "DROP TABLE IF EXISTS history";
			String sql = MySQLDBUtil.DROP + "history";
			stmt.executeUpdate(sql);

			sql = "DROP TABLE IF EXISTS categories";
//			sql = MySQLDBUtil.DROP + "users";
			stmt.executeUpdate(sql);
...
//			sql = MySQLDBUtil.DROP + "categories";
			stmt.executeUpdate(sql);
